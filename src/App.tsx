...
function App() {
  ...
  let {solver, setBlock, route, clearRoute} = UseSolve(blocks, setBlocks, finish, start, col, row);
  ...
  return (
    <div className="App flex flex-col w-max">
        ...
        {route.map((route, index)=>{
          return <div key={index} className='absolute w-10 h-10 z-[1] pointer-events-none' style={{transform: `translate(${(route[0]%col)*48.5}px, ${(Math.floor(route[0]/col))*48.5}px)`}}>
            <div className='relative w-full h-full'>
              ...
              {route[1] === 'left' && <div className='absolute transform -translate-y-1/2 top-1/2 -right-2/3 w-[130%] h-1 bg-yellow-500'></div>}
      ...
      <button className='text-white bg-blue-500 p-2 rounded-md font-bold mt-5 w-11/12 self-center' onClick={()=>solver()}>
        Solve
      </button>
      <button className='text-white bg-red-500 p-2 rounded-md font-bold mt-5 w-11/12 self-center' onClick={()=>{setBlocks(Array(col*row).fill(0).map((_, index)=>new MazeBlock())); clearRoute()}}>
        Clear
      </button>
      <div className='w-11/12 self-center mt-4'>
        <div className='text-black w-full'>Columns</div>
        <input type='number' className='text-black p-2 rounded-md font-bold w-full border border-black' value={col} onFocus={(e)=>{e.target.blur()}} onChange={(e)=>{setCol(Number(e.target.value))}}/>
      </div>
      <div className='w-11/12 self-center mt-4'>
        <div className='text-black w-full'>row</div>
        <input type='number' className='text-black p-2 rounded-md font-bold w-full border border-black' value={row} onFocus={(e)=>{e.target.blur()}} onChange={(e)=>{setRow(Number(e.target.value))}}/>
      </div>
    </div>
  );
}
